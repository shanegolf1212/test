...
// Set up session with FileStore and debugging
const sessionOptions = {
  store: new FileStore({
    path: './sessions', // Ensure this path exists and is writable
    logFn: (message) => console.log('FileStore:', message) // Log FileStore operations
  }),
...
app.use(session({
    store: new FileStore({
        path: './sessions', // Ensure this path exists and is writable
        logFn: (message) => console.log('FileStore:', message), // Log FileStore operations
        retries: 5 // Increase retries to handle temporary I/O errors
    }),
    secret: process.env.SESSION_SECRET,
    resave: false,
...
// Middleware to log all incoming requests and session details
...
// Debugging session storage and retrieval
app.use((req, res, next) => {
  console.log(`Session ID: ${req.sessionID}`);
  console.log(`Session stored for user: ${req.session.user ? req.session.user.email : 'No user session'}`);
  next();
});
...
app.get('/api/auth-check', (req, res) => {
    ...
    if (req.session && req.session.user) {
        console.log(`Authenticated session for user: ${req.session.user.email}`);
        res.json({
            authenticated: true,
...
app.post('/api/sign-in', async (req, res) => {
    ...
    try {
        ...
        req.session.save((err) => {
            ...
            console.log('Session ID after sign-in:', req.sessionID); // Log session ID
